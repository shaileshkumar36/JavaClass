package JavaClass.Class10;

public class BackTracking06 {
    public static boolean isSafe(int sudoku[][] , int row,int col,int digit){

        //clumn
        for(int i=0;i<9;i++){
            if(sudoku[i][col]==digit){
                return false;
            }
        }
        //row
        for(int i =0;i<9;i++){
            if(sudoku[row][i]==digit){
                return false;
            }
        }
        //grid
        int sr = (row/3)*3;
        int sc = (col/3)*3;
        for(int i=0;i<sr+3;i++){
            for(int j=sc;j<sc+3;j++){
                if(sudoku[i][j] == digit){
                    return false;
                }
            }
        }
        return true;
    }

    public static Boolean findSudoku(int sudoku[][] , int row ,int col){
        if(row == 9){
            return true;
        }
        int nexRow = row , nextCol =col+1;
        if(col +1 == 9){
            nexRow = row+1;
            nextCol = 0;
        }
        if(sudoku[row][col] != 0){
            return findSudoku(sudoku, nexRow, nextCol);
        }
        for(int digit =1;digit<=9;digit++){
            if(isSafe(sudoku, row,col,digit)){
                sudoku[row][col] = digit;
                if(findSudoku(sudoku, nexRow, nextCol)){
                    return true;
                }
                sudoku[row][col] = 0;
            }
        }
        return false;
    }

    public static void printSudoku(int sudoku[][]){
        for(int i =0;i<9;i++){
            for(int j=0;j<9;j++){
                System.out.println(sudoku[i][j]);
            }System.out.println();
        }
    }
    public static void main(String args[]){
        
        int sudoku[][] = {
            {0,0,8,0,0,0,0,0,0},
            {4,9,0,1,5,7,0,0,2},
            {0,0,3,0,0,4,1,9,0},
            {1,8,5,0,6,0,0,2,0},
            {0,0,0,0,2,0,0,6,0},
            {9,6,0,4,0,5,3,0,0},
            {0,3,0,0,7,2,0,0,4},
            {0,4,9,0,3,0,0,5,7},
            {8,2,7,0,0,9,0,1,3}
        };

        if(findSudoku(sudoku, 0, 0)){
            System.out.println("s;lkajsd");
            printSudoku(sudoku);
        }else{
            System.out.println("not exgit solutojl");
        }
    }
}
